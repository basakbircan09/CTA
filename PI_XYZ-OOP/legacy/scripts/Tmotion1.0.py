...
def main():
    ...
    try:
        ...
        for axis in REFERENCE_ORDER:
            ...
            # 1. Configure the stage on the controller (CST command)
            # This is the crucial step that synchronizes the controller's parameters.
            print(f"  - Configuring stage '{config['stage']}' for axis {axis}...")
            pidevice.CST(ax, config['stage'])
            time.sleep(0.1)  # Brief pause for controller to load parameters

            # 2. Enable Servo Control
            print(f"  - Enabling servo for axis {axis}...")
            pidevice.SVO(ax, True)

            # 3. Start the Referencing Move
            ...
            # 4. Move slightly off the limit switch
